...
path = "/archive/Raphael.Dussin/FMS2019.01.03_devgfdl_20210308/CM4_piControl_c192_OM4p125_v3/" + \
...
path_vmo = "/archive/Raphael.Dussin/FMS2019.01.03_devgfdl_20210308/CM4_piControl_c192_OM4p125_v3/" + \
...
for i in range(16, 30):  #len(filelist)):
    ...
    if(i%2 == 0):
        ...
        vmo_2d = (ds_vmo['vmo'].isel(yq=slice(1,2241),time=slice(5, 10)).mean('time')).sum('z_l')
        umo_2d = (ds_umo['umo'].isel(xq=slice(1,2881),time=slice(5, 10)).mean('time')).sum('z_l')

    # Save data
    ds_save = (ds_full).mean('time')
    ds_save = xr.merge([ds_save, vmo_2d, umo_2d, eta_dt])

    save_file = save_path + "OM4p125/OM4p125_" + tim_yr[0:4] + "_" + tim_yr[7:11] + ".nc"
